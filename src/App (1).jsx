...
export default function App() {
  ...
  const openEditRec = (rec) => {
    ...
    setEditDurVal(rec.durVal || 0);
  };

  const saveEditRec = async () => {
    const newDurVal = parseFloat(editDurVal);
    const oldDurVal = editRec.durVal || 0;
    const diff = +(newDurVal - oldDurVal).toFixed(1);
    const emp = employees.find(e => e.id === editRec.empId);

    if (emp && diff !== 0) {
      const isLeave = editRec.type.includes("請假");
      const isOT    = editRec.type.includes("補休（加班）");
      if (isLeave) {
        // 請假改多了 → 多扣；改少了 → 退回
        const result = calcDeduct(emp, diff > 0 ? diff : 0);
        let na = emp.annualDays, nc = emp.compDays;
        if (diff > 0) {
          if (!result.ok) { notify("假期不足，無法增加天數", "error"); return; }
          na = result.newAnnual; nc = result.newComp;
        } else {
          // 退回 |diff| 天，先補 annual
          const back = Math.abs(diff);
          na = +(na + back).toFixed(1);
        }
        await updateEmployeeDays(emp.id, na, nc);
      } else if (isOT) {
        // 補休改多了 → 多加；改少了 → 扣回
        const nc = Math.max(0, +(emp.compDays + diff).toFixed(1));
        await updateEmployeeDays(emp.id, emp.annualDays, nc);
      }
    }

    await updateLeaveRecord(editRec.id, {
      note: editNote, date: editDate, duration: editDuration, durVal: newDurVal,
    });
    notify("紀錄已更新，天數同步調整");
    setEditRec(null);
  };
  ...
  const deleteRecord = async (rec) => {
    if (!window.confirm(`確定要刪除「${rec.empName} ${rec.type} ${rec.duration}」？\n天數將自動還原。`)) return;
    const emp = employees.find(e => e.id === rec.empId);
    const dur = rec.durVal || 0;
    if (emp && dur > 0) {
      const isLeave = rec.type.includes("請假");
      const isOT    = rec.type.includes("補休（加班）");
      if (isLeave) {
        // 還原請假天數（加回 annual）
        await updateEmployeeDays(emp.id, +(emp.annualDays + dur).toFixed(1), emp.compDays);
      } else if (isOT) {
        // 扣回補休天數
        await updateEmployeeDays(emp.id, emp.annualDays, Math.max(0, +(emp.compDays - dur).toFixed(1)));
      }
    }
    await deleteLeaveRecord(rec.id);
    notify("已刪除紀錄，天數已還原", "error");
  };
  ...
  const DateDurPicker = ({ dateLabel, dur, setDur, customDays, setCustomDays, dateStart, setDateStart }) => (
        ...
        <div style={{ ...S.landingCard, maxWidth:660 }}>
          ...
          {detailView === "records" && (
            myRecs.length === 0 ? <div style={S.empty}>目前沒有紀錄</div> : (
              <div style={S.tableWrap}>
                <table style={S.table}>
                  <thead><tr>{["類型","日期","天數","備註"].map(h => <th key={h} style={S.th}>{h}</th>)}</tr></thead>
                  <tbody>
                    {myRecs.map(r => (
          ...
          {editRec && (
            <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.4)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:1000 }}>
              <div style={{ background:"#fff", borderRadius:16, padding:28, width:"100%", maxWidth:420, boxShadow:"0 20px 60px rgba(0,0,0,0.25)" }}>
                ...
                {(editRec.type.includes("請假") || editRec.type.includes("補休（加班）")) && (
                  <p style={{ fontSize:12, color:"#0369a1", marginBottom:16 }}>修改天數後，系統會自動更新假期餘額。</p>
                )}
                <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                  <div style={S.fg}>
                  ...
                  {(editRec.type.includes("請假") || editRec.type.includes("補休（加班）")) && (
                    <div style={S.fg}>
                      <label style={S.label}>實際天數（會影響餘額）</label>
                      <input type="number" min="0.5" step="0.5" value={editDurVal}
                        onChange={e => setEditDurVal(e.target.value)} style={S.input} />
                    </div>
                  )}
                  <div style={S.fg}>
